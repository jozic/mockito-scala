...
object Utils {
  ...
  def hasMatchers(using Quotes)(args: List[quotes.reflect.Term]): Boolean =
    args.exists(arg => isMatcher(arg))

  /** Check if an expression is a Mockito matcher */
  def isMatcher(using Quotes)(arg: quotes.reflect.Term): Boolean = {
    import quotes.reflect.*

    // Unwrap Inlined/Block wrappers from Scala 3 inline expansion before checking
    arg match {
      case Inlined(_, _, expansion) => return isMatcher(expansion)
      case Block(_, expr)           => return isMatcher(expr)
      case _                        => // continue with existing logic
    }

    // Check if the tree representation contains MacroMatchers
    if arg.show.contains("org.mockito.matchers.MacroMatchers") then return true
  ...
  def transformArgs(using Quotes)(args: List[quotes.reflect.Term], matcherValNames: Set[String] = Set.empty): List[quotes.reflect.Term] =
   ...
   */
  def transformArgsForApply(using
      Quotes
  )(
      fun: quotes.reflect.Term,
      args: List[quotes.reflect.Term],
      hoistedBindings: scala.collection.mutable.ListBuffer[quotes.reflect.Statement],
    ...
    if paramInfo.isEmpty then return transformed
   ...
   */
  def transformArg(using Quotes)(arg: quotes.reflect.Term, matcherValNames: Set[String] = Set.empty): quotes.reflect.Term = {
    import quotes.reflect.*

    val isArgMatcher = isMatcher(arg) || isMatcherValRef(arg, matcherValNames)

    if isArgMatcher then {
      arg
    } else {
      arg match {
  ...
  private def isMatcherValRef(using Quotes)(arg: quotes.reflect.Term, matcherValNames: Set[String]): Boolean = {
    import quotes.reflect.*
    if matcherValNames.isEmpty then return false
    arg match {
  ...
  def detectMatcherValDefs(using Quotes)(stats: List[quotes.reflect.Statement]): Set[String] = {
    import quotes.reflect.*
    stats.collect {
   ...
   */
  def inlineMatcherValDefs(using
      Quotes
  )(
    ...
    import quotes.reflect.*
    if matcherValNames.isEmpty then return (stats, expr)
  ...
  def packageName(using Quotes)(className: String): String =
    if className.contains("Scalaz") then "scalaz" else "cats"

  /** Determine class name for cats/scalaz based on class name */
  def className(using Quotes)(className: String, start: String): String =
    if className.contains("Scalaz") then start + "Scalaz" else start + "Cats"
}
