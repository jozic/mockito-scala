...
case class ScalazStubbing[F[_], T](delegate: OngoingStubbing[F[T]]) extends ScalazStubbingBase[F, T] {
  // In Scala 3, String can be enriched to Function1[Int, Char] through WrappedString, so the Function1
  // member may match String values. We add a runtime guard to handle this case.
